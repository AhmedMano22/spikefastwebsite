...
@Component({
  selector: 'app-copy-service',
  templateUrl: './copy-service.component.html',
  styleUrls: ['./copy-service.component.css']
})
export class CopyServiceComponent implements OnInit {
  ...
  private userTest = { copyServices_Id: '', user_Id: '' ,statue:'' };
  ...
  GetDataPagination(page: any) {
    this.progressRef.start();
    this.subscriptions.push(this.apiService.GetAll('CopyServices',page,10).subscribe(
      res => {
  ...
  create(id)
  {
    ...
    if(statue){
      this.userTest.copyServices_Id = id;
      this.userTest.statue = '1';
      this.userTest.user_Id = JSON.parse(localStorage.getItem('id')!) ;
      this.apiService.create_user_Subscripton(this.userTest).subscribe((res) => {
        console.log(res);
